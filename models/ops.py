...
def write_loss_scalars(epoch, batch_idx, loss, log_prior, log_variational_posterior, negative_log_likelihood, writer,
                       NUM_BATCHES):
...
class BayesLinearLayer(nn.Module):
    ...
    def bayes_forward(self, input, sample=False, calculate_log_probs=False):
    ...
    def forward(self, input, sample=False, calculate_log_probs=False):
        return self.bayes_forward(input, sample, calculate_log_probs)
...
class BayesConvLayer(nn.Module):
    ...
    def bayes_forward(self, input, sample=True, calculate_log_probs=False):
        if self.training or sample:
            weight = self.weight.sample()
            bias = self.bias.sample()
        else:
